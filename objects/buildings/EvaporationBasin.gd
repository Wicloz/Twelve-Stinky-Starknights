...
func _upgrade_research() -> Array[ResearchItem]:
	...
	var cascade := _output_upgrade(
		...
		2, {Stockpile.ItemType.BRICKS: 200, Stockpile.ItemType.FLUID_HARDWARE: 8}, basins)
	...
	var solar := _speed_upgrade(
		...
		1.5, {Stockpile.ItemType.MECHANICAL_COMPONENTS: 50, Stockpile.ItemType.PLASTIC: 90})
	var mvr := _speed_upgrade(
		2, "Vapor Recompression",
		"Recompress the vapour and pump its own heat back into the brine.",
		1.5, {Stockpile.ItemType.ELECTRONIC_ACTUATORS: 25, Stockpile.ItemType.POWER_CELLS: 30}, solar)
	...
	var recirc := _efficiency_upgrade(
		...
		1.5, {Stockpile.ItemType.FLUID_HARDWARE: 8})
	var multi_effect := _efficiency_upgrade(
		...
		1.5, {Stockpile.ItemType.CUPRONICKEL_INGOTS: 120, Stockpile.ItemType.POWER_CELLS: 25}, recirc)
