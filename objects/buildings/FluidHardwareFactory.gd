...
func _upgrade_research() -> Array[ResearchItem]:
	...
	var boring := _output_upgrade(
		...
		2, {Stockpile.ItemType.MECHANICAL_COMPONENTS: 80, Stockpile.ItemType.CUPRONICKEL_INGOTS: 90})
	...
	var test_bench := _output_upgrade(
		...
		2, {Stockpile.ItemType.FLUID_HARDWARE: 12, Stockpile.ItemType.WATER: 150}, boring)
	...
	var turret := _speed_upgrade(
		...
		1.5, {Stockpile.ItemType.RAW_TITANIUM: 50, Stockpile.ItemType.MECHANICAL_COMPONENTS: 60}, boring)
	var cnc := _speed_upgrade(
		2, "CNC Machining Centres",
		"Circuit-controlled machining centres run the whole tool path unattended.",
		1.5, {Stockpile.ItemType.INTEGRATED_CIRCUITS: 60, Stockpile.ItemType.ELECTRONIC_ACTUATORS: 25}, turret)
	...
	var seals := _efficiency_upgrade(
		...
		1.5, {Stockpile.ItemType.PLASTIC: 120, Stockpile.ItemType.ACRYLIC: 90}, lapping)
