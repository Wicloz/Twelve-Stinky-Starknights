...
func _upgrade_research() -> Array[ResearchItem]:
	...
	var kettle := _output_upgrade(
		...
		2, {Stockpile.ItemType.CUPRONICKEL_INGOTS: 75})
	var continuous := _output_upgrade(
		...
		2, {Stockpile.ItemType.FLUID_HARDWARE: 8, Stockpile.ItemType.MECHANICAL_COMPONENTS: 40}, kettle)
	...
	var pressure := _speed_upgrade(
		...
		1.5, {Stockpile.ItemType.FLUID_HARDWARE: 7, Stockpile.ItemType.CUPRONICKEL_INGOTS: 45})
	var flash := _speed_upgrade(
		2, "Flash Extraction",
		"Flash-heat the slurry and extract in seconds rather than minutes.",
		1.5, {Stockpile.ItemType.ELECTRONIC_ACTUATORS: 25, Stockpile.ItemType.POWER_CELLS: 25}, pressure)
	...
	var recirc := _efficiency_upgrade(
		...
		1.5, {Stockpile.ItemType.WATER: 150, Stockpile.ItemType.MECHANICAL_COMPONENTS: 40})
	var concentrate := _efficiency_upgrade(
		...
		1.5, {Stockpile.ItemType.POWER_CELLS: 30, Stockpile.ItemType.PLASTIC: 75}, recirc)
	...
	var control_suite := _speed_upgrade(
		...
		1.5, {Stockpile.ItemType.INDUSTRIAL_CONTROLLERS: 15, Stockpile.ItemType.ELECTRONIC_COMPONENTS: 60},
