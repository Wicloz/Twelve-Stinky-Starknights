...
func _upgrade_research() -> Array[ResearchItem]:
	var directional := _yield_upgrade(
		...
		2, {Stockpile.ItemType.FLUID_HARDWARE: 6, Stockpile.ItemType.MECHANICAL_COMPONENTS: 40})
	var top_drive := _speed_upgrade(
		2, "Top Drive",
		"A powered top drive spins pipe continuously instead of stopping to re-set.",
		1.5, {Stockpile.ItemType.FLUID_HARDWARE: 6, Stockpile.ItemType.POWER_CELLS: 25})
	var fracturing := _yield_upgrade(
		1, "Hydraulic Fracturing",
		"Pump in a mountain of sand to prop the fractured rock open and free the flow.",
		2, {Stockpile.ItemType.SAND: 100, Stockpile.ItemType.FLUID_HARDWARE: 6}, [directional, top_drive])
	var managed_pressure := _speed_upgrade(
		...
		1.5, {Stockpile.ItemType.ELECTRONIC_ACTUATORS: 25, Stockpile.ItemType.FLUID_HARDWARE: 5}, [directional, top_drive])
	var multilateral := _yield_upgrade(
		...
		2, {
			Stockpile.ItemType.INDUSTRIAL_CONTROLLERS: 12,
			Stockpile.ItemType.ELECTRONIC_ACTUATORS: 30,
