...
func _upgrade_research() -> Array[ResearchItem]:
	var intake := _yield_upgrade(
		...
		2, {Stockpile.ItemType.CUPRONICKEL_INGOTS: 60})
	var boosters := _speed_upgrade(
		2, "Booster Pumps",
		"In-line booster pumps push each cycle through the mains faster.",
		1.5, {Stockpile.ItemType.FLUID_HARDWARE: 6, Stockpile.ItemType.MECHANICAL_COMPONENTS: 35})
	var wellfield := _yield_upgrade(
		1, "Deep Well Field",
		"Sink and brick-case a whole field of deep wells to tap far more of the aquifer.",
		2, {Stockpile.ItemType.BRICKS: 120, Stockpile.ItemType.FLUID_HARDWARE: 7}, [intake, boosters])
	var turbopumps := _speed_upgrade(
		2, "High-Head Turbopumps",
		"High-head turbopumps lift each batch of water in seconds.",
		1.5, {Stockpile.ItemType.ELECTRONIC_ACTUATORS: 25, Stockpile.ItemType.POWER_CELLS: 25}, [intake, boosters])
