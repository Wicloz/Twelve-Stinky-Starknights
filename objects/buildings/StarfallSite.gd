...
func _upgrade_research() -> Array[ResearchItem]:
	var array := _yield_upgrade(
		...
		2, {Stockpile.ItemType.MECHANICAL_COMPONENTS: 20, Stockpile.ItemType.FLUID_HARDWARE: 15})
	var drones := _speed_upgrade(
		2, "Recovery Drones",
		"A swarm of autonomous drones recovers the scattered fragments far faster.",
		1.5, {
			Stockpile.ItemType.ELECTRONIC_ACTUATORS: 8,
			Stockpile.ItemType.POWER_CELLS: 8,
			Stockpile.ItemType.ELECTRONIC_COMPONENTS: 10,
		})
	var crater := _yield_upgrade(
		...
		2, {Stockpile.ItemType.MECHANICAL_COMPONENTS: 30, Stockpile.ItemType.FLUID_HARDWARE: 15}, [array, drones])
	var separators := _speed_upgrade(
		...
		1.5, {Stockpile.ItemType.ELECTRUM_WIRE: 40}, [array, drones])
	var grid := _yield_upgrade(
		...
		2, {
			Stockpile.ItemType.INDUSTRIAL_CONTROLLERS: 5,
			Stockpile.ItemType.ELECTRONIC_COMPONENTS: 20,
			Stockpile.ItemType.POWER_CELLS: 10,
		}, [crater, separators])
